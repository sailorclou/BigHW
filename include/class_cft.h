...
struct cfgGroup {
	...
	int item_num;                    // 项目数
	vector<string> items;    // 各项内容
	/*cfgGroup(string name, int item_num, string item)
	{
		group_name = name;
		item_num = item_num;
...
private:
	...
	vector<cfgGroup> groups;
	string trim(const std::string& str) const;
	bool get_key_value(string item, string& key, string& value) const;
	bool is_valid_ip(const string& str_ipaddr) const;
	const unsigned int get_int_ipaddr(const string& str_ipaddr) const;
public:
	/* 构造函数的作用：将指定配置文件中的所有组/项均读入并存储到自定义结构中，方便后续各种get函数的调用 */
	config_file_tools(const char* const _cfgname, const enum BREAK_CTYPE _bctype = BREAK_CTYPE::Equal);
	config_file_tools(const string& _cfgname, const enum BREAK_CTYPE _bctype = BREAK_CTYPE::Equal);
